...
public class NewDoseActivity extends AppCompatActivity {
    ...
    public void traiterIntent() {
        ...
        if (intent.hasExtra("doseToUpdate")) {
            doseToUpdate = (Dose) intent.getSerializableExtra("doseToUpdate");
            textNom.setText(doseToUpdate.getName());
            textDescription.setText(doseToUpdate.getDetail());
        }
    }

    public void retourPagePrecedente() {
        Intent nextActivity = new Intent(NewDoseActivity.this,pagePrecedente);
        nextActivity.putExtra("precedent", NewDoseActivity.class);
    ...
    public boolean isExistant(TextView textView) {
        ...
        for (Dose dose : listAllDose) {
            if (doseToUpdate == null) {
                ...
                if (!doseToUpdate.getName().equals(dose.getName()) && textView.getText().toString().equals(dose.getName())) {
                    textView.requestFocus();
                    textView.setError("la dose existe déjà");
                    reponse = true;
                }
